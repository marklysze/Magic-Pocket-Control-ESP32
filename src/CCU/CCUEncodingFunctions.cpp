...
/*
    public static CCUPacketTypes.Command CreateVideoWhiteBalanceCommand(short whiteBalance, short tint)
    {
        short[] dataArray = { whiteBalance, tint };
        byte[] payloadData = UtilityFunctions.ToByteArrayFromArray(dataArray);

        CCUPacketTypes.Command command = new CCUPacketTypes.Command(
            CCUPacketTypes.kBroadcastTarget,
            CCUPacketTypes.CommandID.ChangeConfiguration,
            CCUPacketTypes.Category.Video,
            (byte)CCUPacketTypes.VideoParameter.ManualWB,
            CCUPacketTypes.OperationType.AssignValue,
            (byte)CCUPacketTypes.DataTypes.kInt16,
            data: payloadData);

        return command;
    }

    public static CCUPacketTypes.Command CreateVideoSetAutoWBCommand() {
        return CreateVoidCommand(CCUPacketTypes.Category.Video, (byte)CCUPacketTypes.VideoParameter.SetAutoWB);
    }

    public static CCUPacketTypes.Command CreateRecordingFormatCommand(CCUPacketTypes.RecordingFormatData recordingFormatData)
    {
        short flags = 0;
        if(recordingFormatData.mRateEnabled) {
            flags = (short)((int)flags | (int)CCUPacketTypes.VideoRecordingFormat.FileMRate);
        }

        if(recordingFormatData.offSpeedEnabled) {
            flags = (short)((int)flags | (int)CCUPacketTypes.VideoRecordingFormat.SensorOffSpeed);
        }

        if(recordingFormatData.interlacedEnabled){
            flags = (short)((int)flags | (int)CCUPacketTypes.VideoRecordingFormat.Interlaced);
        }

        if(recordingFormatData.windowedModeEnabled) {
            flags = (short)((int)flags | (int)CCUPacketTypes.VideoRecordingFormat.WindowedMode);
        }
        ...
        byte[] payloadData = UtilityFunctions.ToByteArrayFromArray(data);

        CCUPacketTypes.Command command = new CCUPacketTypes.Command(
            CCUPacketTypes.kBroadcastTarget,
            CCUPacketTypes.CommandID.ChangeConfiguration,
            CCUPacketTypes.Category.Video,
            (byte)CCUPacketTypes.VideoParameter.RecordingFormat,
            CCUPacketTypes.OperationType.AssignValue,
            (byte)CCUPacketTypes.DataTypes.kInt16,
            payloadData);
    ...
    // TEST TEST - TRYING TO GET INFO RATHER THAN CHANGE - TEST TEST
    public static CCUPacketTypes.Command CreateRecordingFormatStatusCommand()
    {
        byte[] data = UtilityFunctions.GetBytes(400);
        CCUPacketTypes.Command command = new CCUPacketTypes.Command(
            CCUPacketTypes.kBroadcastTarget,
            CCUPacketTypes.CommandID.ChangeConfiguration,
            CCUPacketTypes.Category.Video,
            (byte)CCUPacketTypes.VideoParameter.ISO,
            CCUPacketTypes.OperationType.OffsetValue,
            (byte)getCCUDataType((int)0),
            data);
    ...
    private static CCUPacketTypes.Command CreateVoidCommand(CCUPacketTypes.Category category, byte parameter){
        byte[] data = new byte[] { };
        CCUPacketTypes.Command command = new CCUPacketTypes.Command(
            CCUPacketTypes.kBroadcastTarget,
            CCUPacketTypes.CommandID.ChangeConfiguration,
            ...
            parameter,
            CCUPacketTypes.OperationType.AssignValue,
            (byte)CCUPacketTypes.DataTypes.kVoid,
            data);
    ...
    public static CCUPacketTypes.Command CreateFixed16Command(short value, CCUPacketTypes.Category category , byte parameter) {
        byte[] data = UtilityFunctions.GetBytes(value);
        CCUPacketTypes.Command command = new CCUPacketTypes.Command(
            CCUPacketTypes.kBroadcastTarget,
            CCUPacketTypes.CommandID.ChangeConfiguration,
            ...
            parameter,
            CCUPacketTypes.OperationType.AssignValue,
            (byte)CCUPacketTypes.DataTypes.kFixed16,
            data: data);
    ...
    public static CCUPacketTypes.Command CreateCommand<T>(T value, CCUPacketTypes.Category category, byte parameter)
    where T : struct
    {
        byte[] data = UtilityFunctions.GetBytes(value);
        CCUPacketTypes.Command command = new CCUPacketTypes.Command(
            ...
            parameter,
            CCUPacketTypes.OperationType.AssignValue,
            (byte)getCCUDataType(value),
            data: data);

        return command;
    }

    private static CCUPacketTypes.DataTypes getCCUDataType<T>(T value)
    {
        ...
        throw new Exception("Have not handled type in CCUEncodingFunction.getCCUDataType");
    }

    // Video category encoding functions
    public static CCUPacketTypes.Command CreateVideoSensorGainCommand(byte value)
    {
        return CreateCommand(value, CCUPacketTypes.Category.Video, (byte)CCUPacketTypes.VideoParameter.SensorGain);
    }

    // MS
    public static CCUPacketTypes.Command CreateVideoISOCommand(int value)
    {
        return CreateCommand(value, CCUPacketTypes.Category.Video, (byte)CCUPacketTypes.VideoParameter.ISO);
    }

    public static CCUPacketTypes.Command CreateTransportInfoCommand(TransportInfo transportInfo)
    {
        ...
        CCUPacketTypes.Command command = new CCUPacketTypes.Command(
            ...
            CCUPacketTypes.CommandID.ChangeConfiguration,
            CCUPacketTypes.Category.Media,
            (byte)CCUPacketTypes.MediaParameter.TransportMode,
            CCUPacketTypes.OperationType.AssignValue,
            (byte)CCUPacketTypes.DataTypes.kInt8,
            data);
